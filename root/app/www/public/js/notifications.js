...
// ---------------------------------------------------------------------------------------------
function previewNotificationPayload(el)
{
    pageLoadingStart();
    $.ajax({
        url: BASE_URL + 'ajax/notifications.php',
        type: 'post',
        dataType: 'json',
        data: '&event=previewPayload&name=' + encodeURIComponent($(el).attr('data-name') || '') + '&payload=' + encodeURIComponent($(el).attr('data-payload') || '{}'),
        success: function (response) {
            pageLoadingStop();
            if (!response || response.error) {
                toast(translate('notifications'), (response && response.message) || translate('unableToLoadPage'), 'error');
                return;
            }
            dialogOpen({
                id: 'notification-payload',
                title: response.title || translate('event'),
                size: 'md',
                footer: false,
                body: response.body || ''
            });
