...
trait MediaApp
{
    ...
    {
        if (is_array($this->mediaAppsCache)) {
            return $this->mediaAppsCache;
        }

        $mediaApps = [];

        $sql = "SELECT id, name, platform, url, token, apikey, role, sync_mode, active, server_id, server_name, last_scan, needs_sync, webhooks
    ...
    {
        $apps = $this->getMediaApps();
        if (!empty($apps[$id])) {
            return $apps[$id];
        }
        if (!empty($apps[intval($id)])) {
            return $apps[intval($id)];
        }
    ...
    public function addMediaApp($name, $platform, $url, $token, $apikey, $role, $syncMode, $active, $serverId, $serverName)
    {
        $sql = "INSERT INTO " . MEDIA_APP_TABLE . "
                (`name`, `platform`, `url`, `token`, `apikey`, `role`, `sync_mode`, `active`, `server_id`, `server_name`, `last_scan`, `needs_sync`)
                VALUES
                ('" . $this->prepare($name) . "', " . intval($platform) . ", '" . $this->prepare($url) . "', '" . $this->prepare($token) . "', '" . $this->prepare($apikey) . "', " . intval($role) . ", " . intval($syncMode) . ", " . intval($active) . ", '" . $this->prepare($serverId) . "', '" . $this->prepare($serverName) . "', 0, 0)";
        $res = $this->query($sql);
    ...
    public function setMediaAppWebhooks($id, $enabled)
    {
        $sql                  = "UPDATE " . MEDIA_APP_TABLE . "
                SET webhooks = " . intval($enabled) . "
                WHERE id = " . intval($id);
        $res                  = $this->query($sql);
        $this->mediaAppsCache = null;
    ...
    public function deleteMediaApp($id)
    {
        ...
        $this->deleteMediaAppLibraries($id);
        ...
        unset($this->appLibrariesCache[intval($id)]);
