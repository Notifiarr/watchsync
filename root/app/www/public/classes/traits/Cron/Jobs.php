...
trait Jobs
{
    ...
    public function logSearch($id, $query)
    {
        ...
        foreach ($this->logContentLines($id) as $index => $line) {
            if (mb_stripos((string) $line, $query) !== false) {
    ...
    public function renderLogMatchHtml($lines, $query = '')
    {
        ...
        foreach ($lines as $row) {
            ...
            $html .= '<div class="sync-log-line sync-log-match-line" data-line="' . $line . '">' . $this->highlightLogQuery($text, $query) . '</div>';
        }
    ...
    public function renderLogHtml($lines, $start = 0)
    {
        ...
        foreach ($lines as $index => $line) {
            $html .= '<div class="sync-log-line" data-line="' . ($start + intval($index)) . '">' . htmlEscape($line) . '</div>';
        }
    ...
    public function requeue($id)
    {
        ...
        unset($this->sidecar['stats'], $this->sidecar['runtime'], $this->sidecar['size'], $this->sidecar['queued_wait'], $this->sidecar['dry_run_summary']);
    ...
    public function deleteAllLogs()
    {
        ...
        if (is_dir(CRON_LOGS_PATH)) {
            foreach (glob(CRON_LOGS_PATH . 'pull-*.log') ?: [] as $file) {
                if (is_file($file)) {
                    unlink($file);
                }
            }
    ...
    public function automaticNextRun($key)
    {
        ...
        $last     = $schedule ? $this->database->getLastSyncHistoryJob($schedule['types']) : [];
        $lastAt   = intval($last['queued'] ?? 0);
        $next     = $lastAt ? $lastAt + intval($schedule['interval'] ?? 0) : time() + intval($schedule['interval'] ?? 0);
        $until    = $next > time() ? relativeBetweenDates(time(), $next, true) : '0s';
    ...
    public function queueAutomaticJobs()
    {
        global $mediaApps;
        ...
        foreach ($this->automaticSchedules() as $key => $schedule) {
            ...
            $last   = $this->database->getLastSyncHistoryJob($schedule['types']);
            $lastAt = intval($last['queued'] ?? 0);
            if (!$lastAt || ($lastAt + intval($schedule['interval'])) > time()) {
                continue;
            }
    ...
    public function queueHistoryForNewLibraryData()
    {
        global $mediaApps;
        ...
        $mediaApps->applyNewHistoryUsers();
        $userIds = [];
        foreach ($this->database->getMediaAppUsers($master['id']) as $user) {
            $userIds[] = intval($user['id']);
        }
    ...
    public function cancelAutomaticJobs()
    {
        foreach ($this->database->getSyncHistoryJobs() as $row) {
            $job = $this->jobFromRow($row);
            if (!$job) {
    ...
    public function writeJobHeader()
    {
        ...
        unset($payload['runtime'], $payload['size'], $payload['queued_wait'], $payload['dry_run_summary']);
