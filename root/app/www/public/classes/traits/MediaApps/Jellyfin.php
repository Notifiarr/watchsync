...
trait Jellyfin
{
    protected $jellyfinApiCache = [];

    public function jellyfinApiGet($url, $headers = [])
    {
        $key = $url . "\n" . implode("\n", $headers);
        if (array_key_exists($key, $this->jellyfinApiCache)) {
            return $this->jellyfinApiCache[$key];
        }

        $curl                         = curl($url, $headers, 'GET');
        $this->jellyfinApiCache[$key] = $curl;

        return $curl;
    }

    public function jellyfinApiForget()
    {
        $this->jellyfinApiCache = [];
    }
    ...
    public function jellyfinGetItems($url, $apikey, $libraryKeys = [], $seriesRemoteId = '', $seriesTitle = '', $since = 0)
    {
        ...
        foreach ($keys as $libraryKey) {
            ...
            $libraryRows  = $this->jellyfinItems($url, $apikey, $includeTypes, '', '', $libraryKey, $since);
            $movies       = 0;
            $series       = 0;
            $episodes     = 0;
    ...
    public function jellyfinGetUsers($url, $apikey)
    {
        $url      = rtrim(trim($url), '/');
        $headers  = $this->jellyfinHeaders($apikey);
        $curl     = $this->jellyfinApiGet(sprintf(MediaAppEndpoints::ENDPOINT_JELLYFIN_USERS, $url), $headers);
        $response = is_array($curl['response']) ? $curl['response'] : json_decode($curl['response'], true);
    ...
    {
        $url  = rtrim(trim($url), '/');
        $curl = $this->jellyfinApiGet(sprintf(MediaAppEndpoints::ENDPOINT_JELLYFIN_USERS, $url), $this->jellyfinHeaders($apikey));
