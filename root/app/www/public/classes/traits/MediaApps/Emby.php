...
trait Emby
{
    ...
    public function embyGetItems($url, $apikey, $libraryKeys = [], $seriesRemoteId = '', $seriesTitle = '', $since = 0)
    {
        ...
        foreach ($keys as $libraryKey) {
            ...
            foreach ($this->embyItems($url, $apikey, $includeTypes, '', '', $libraryKey, $since) as $row) {
    ...
    public function embyGetUsers($url, $apikey)
    {
        $url      = rtrim(trim($url), '/');
        $headers  = $this->embyHeaders($apikey);
        $curl     = curl(sprintf(MediaAppEndpoints::ENDPOINT_EMBY_USERS, $url), $headers, 'GET');
        $response = is_array($curl['response']) ? $curl['response'] : json_decode($curl['response'], true);
    ...
    {
        $url  = rtrim(trim($url), '/');
        $curl = curl(sprintf(MediaAppEndpoints::ENDPOINT_EMBY_USERS, $url), $this->embyHeaders($apikey), 'GET');
