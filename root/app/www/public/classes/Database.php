...
class Database
{
    ...
    public function ensureLibraryRemoteUniqueIndexes()
    {
        $tables = [MOVIE_TABLE, SERIES_TABLE, EPISODE_TABLE];
        $fields = ['plex_remote_id', 'emby_remote_id', 'jellyfin_remote_id'];
        foreach ($tables as $table) {
            foreach ($fields as $field) {
                ...
                $res      = $this->query("SHOW INDEX FROM `" . $table . "` WHERE Column_name = '" . $field . "'");
                $keyNames = [];
                $unique   = false;
                while ($row = $this->fetchAssoc($res)) {
                    $keyNames[$row['Key_name']] = true;
                    if (intval($row['Non_unique']) == 0) {
                        $unique = true;
                    }
                }
                if ($unique) {
                    logger(MIGRATION_LOG, $table . '.' . $field . ' already unique');
                    continue;
                }

                foreach (array_keys($keyNames) as $keyName) {
                    $this->query("ALTER TABLE `" . $table . "` DROP INDEX `" . $keyName . "`");
                    if ($dropError = $this->error()) {
                        logger(MIGRATION_LOG, 'drop index ' . $table . '.' . $keyName . ': ' . $dropError);
                    }
                }
                $this->query("ALTER TABLE `" . $table . "`
                        ADD UNIQUE KEY `" . $field . "` (`" . $field . "`)");
                if ($addError = $this->error()) {
                    logger(MIGRATION_LOG, 'add unique ' . $table . '.' . $field . ' failed: ' . $addError);
                    logger(SYSTEM_LOG, 'Failed to add unique index ' . $table . '.' . $field . ': ' . $addError);
                } else {
                    logger(MIGRATION_LOG, 'added unique ' . $table . '.' . $field);
                }
            }
        }
    }
