...
<table class="table table-bordered table-hover table-no-squish">
    ...
    <thead>
        <tr>
            ...
            <th class="sync-history-remove text-center" style="width: 5%;">
    ...
    <tbody>
        ...
        <?php foreach ($jobs as $job) { ?>
                    <tr class="sync-job-row" style="cursor: pointer;" data-id="<?= htmlEscape($job['id']) ?>" data-status="<?= htmlEscape($job['status']) ?>">
                        ...
                        <?php } ?></td>
                        <td class="sync-history-type"><?= htmlEscape($mediaApps->getSyncTypeName($job['sync_type'] ?? MediaSyncTypes::USERS, $job['sync_accounts'] ?? 0)) ?></td>
