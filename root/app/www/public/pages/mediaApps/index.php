...
if ($masterApp) {
    ...
    $masterCounts  = [
        'users'       => count($mediaApps->visibleMediaAppUsers($masterApp)),
...
<div class="row">
    ...
    <?php foreach ($mediaAppRows as $mediaApp) {
        ...
        $userCount     = count($mediaApps->visibleMediaAppUsers($mediaApp));
            ...
            <div class="card border shadow-sm h-100">
                <div class="card-body">
                    ...
                    <div class="small mb-3 d-flex align-items-center">
                        ...
                        <button type="button" class="btn btn-outline-<?= $online ? 'success' : 'danger' ?> btn-sm py-0 px-2 ms-auto" <?php if ($online) { ?> onclick="openSyncLibrary(<?= intval($mediaApp['id']) ?>, <?= htmlspecialchars(json_encode(strval($mediaApp['name'] ?? '')), ENT_QUOTES, 'UTF-8') ?>)" <?php } else { ?> disabled title="<?= htmlEscape(translate('offline')) ?>" <?php } ?>><?= htmlEscape(translate('syncMyMedia')) ?></button>
                        <button type="button" class="btn btn-outline-<?= $online ? 'success' : 'danger' ?> btn-sm py-0 px-2 ms-1" <?php if ($online) { ?> onclick="openSyncHistory(<?= intval($mediaApp['id']) ?>, <?= htmlspecialchars(json_encode(strval($mediaApp['name'] ?? '')), ENT_QUOTES, 'UTF-8') ?>)" <?php } else { ?> disabled title="<?= htmlEscape(translate('offline')) ?>" <?php } ?>><?= htmlEscape(translate('syncHistory')) ?></button>
                    </div>
