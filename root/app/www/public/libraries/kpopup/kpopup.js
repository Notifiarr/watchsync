var popup = function (dom, options) {
    ...
    var width = cssSize(options.width, '400px');
    var height = options.height !== undefined && options.height !== null && options.height !== ''
        ? cssSize(options.height, '100%')
        : '';
    ...
    this.popupTop = function () {
        width = width || cssSize('', '350px');
        height = height || cssSize('', '250px');
        leftCss = rightCss || (leftCss || 'left:0;');
        bottomCss = 'bottom:auto;';
        topCss = 'top: -' + height + ';';
