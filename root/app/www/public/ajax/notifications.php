...
switch ($_POST['event'] ?? '') {
            ...
            <table class="table table-bordered table-hover table-no-squish">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($linkRow) { ?>
                            <i class="far fa-bell" style="cursor: pointer;" title="<?= htmlEscape(translate('testNotification')) ?>" onclick="testNotify(<?= intval($linkRow['id']) ?>, '<?= htmlEscape($notificationTrigger['name']) ?>')"></i>
                            <?php } ?>
                            <?= htmlEscape($notificationTrigger['label']) ?>
                        </td>
                        <td><?= htmlEscape($notificationTrigger['description']) ?></td>
                        <td><?= htmlEscape($notificationTrigger['event']) ?></td>
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
    ...
    case 'test':
        ...
        echo json_encode([
            'error'   => true,
            'message' => translate('testNotificationFailed') . ($apiResponse['error'] ? ' ' . $apiResponse['error'] : ''),
        ]);
        exit;
