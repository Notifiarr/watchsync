    ...
    switch ($event) {
        ...
        case 'saveSyncSettings':
            ...
            $database->setSettings([
                ...
                'automaticParity'          => !empty($_POST['automaticParity']) ? '1' : '',
                'automaticLibrary'         => !empty($_POST['automaticLibrary']) ? '1' : '',
                'automaticHistory'         => !empty($_POST['automaticHistory']) ? '1' : '',
        ...
        case 'saveLoginSettings':
            $mode          = normalizeLoginMode($_POST['loginMode'] ?? LoginModes::REQUIRED);
            $allowLoopback = !empty($_POST['loginAllowLoopback']) ? '1' : '';
            $allowPrivate  = !empty($_POST['loginAllowPrivate']) ? '1' : '';
            $upstreams     = trim(strval($_POST['loginUpstreams'] ?? ''));
            $authHeader    = normalizeLoginAuthHeader($_POST['loginAuthHeader'] ?? LOGIN_DEFAULT_AUTH_HEADER);
            if ($mode == LoginModes::BYPASS) {
                $networks = array_merge(
                    loginBuiltinNetworks($allowLoopback == '1', $allowPrivate == '1'),
                    parseLoginUpstreams($upstreams),
                );
                if (!$networks) {
                    $result = [
                        'error'   => true,
                        'message' => translate('loginUpstreamsRequired'),
                    ];
                    break;
                }
            }
            ...
            $credentialMsg = translate('saved');
            try {
                $credentialMsg = $user->saveOwnSettings(
                    $uid,
                    $_POST['username'] ?? '',
            ...
            $database->setSettings([
                'loginMode'          => $mode,
                'loginAllowLoopback' => $allowLoopback,
                'loginAllowPrivate'  => $allowPrivate,
                'loginUpstreams'     => $upstreams,
                'loginAuthHeader'    => $authHeader,
            ]);
            $result = [
                'error'   => false,
