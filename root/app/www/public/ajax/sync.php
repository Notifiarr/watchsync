...
switch ($_POST['event'] ?? '') {
    case 'toggleAutomatic':
        ...
        $enabled = !$cron->automaticEnabled($key);
        $cron->setAutomaticEnabled($key, $enabled);
    ...
    case 'logViewer':
        ...
        $limit   = $cron->logFitsViewer($logFile) ? PHP_INT_MAX : 50;
        $chunk   = $cron->logLines($job['id'], -1, $limit);
    ...
    case 'logSearch':
        $id    = $_POST['id'] ?? '';
        $query = $_POST['query'] ?? '';
        $limit = $cron->logFitsViewer($cron->logFile($id)) ? PHP_INT_MAX : 100;
